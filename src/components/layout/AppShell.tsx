...
import { LogOut, ChevronDown, User, Copy, Check, Menu, X } from 'lucide-react';
        ...
        <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-slate-200 py-2 z-50">
          ...
          <div className="px-4 py-3 border-b border-slate-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={copyInviteCode} className="p-1.5 text-slate-400 hover:text-brand-500 rounded hover:bg-slate-100 transition-colors flex-shrink-0">
                {copied ? <Check size={13} className="text-green-500" /> : <Copy size={13} />}
              </button>
            </div>
          </div>
          <button onClick={() => setOpen(false)} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
            <User size={14} className="text-slate-400" /> Profile
          </button>
          <button onClick={() => { setOpen(false); signOut(); }} className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
  ...
  return (
    <div className="flex min-h-screen w-full bg-surface">
      ...
      <div className="flex-1 lg:ml-60 flex flex-col min-h-screen">
        {/* Top bar */}
        <header className="sticky top-0 z-20 flex items-center gap-3 px-4 sm:px-6 py-3 bg-white border-b border-surface-border">
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 min-h-full">
            <Outlet />
          </div>
        </main>
      </div>
